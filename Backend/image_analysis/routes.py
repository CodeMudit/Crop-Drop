...
async def analyze_image_endpoint(
    ...
    try:
        ...
        lang_map = {
            'hi': 'colloquial Hinglish',
            'hinglish': 'colloquial Hinglish',
            'en': 'English',
            'pa': 'Punjabi'
            # Add more mappings as needed, e.g., 'te': 'Telugu'
        }
        ...
        # Craft prompt for Gemini (concise, focused)
        detailed_prompt = (
            f"Briefly describe {cleaned_result} disease in {prompt_lang}: "
            f"what it is, treatment, cure, and fertilizer suggestions. "
        ...
        if voice:
            ...
            voice_filename = generate_voice(detailed_info, lang=user_lang)  # No SSML for image analysis
            voice_end = time.time()
